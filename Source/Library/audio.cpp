...
namespace game_framework {
    ...
    CAudio::CAudio() {
        ...
        DWORD dwThreadID;
        HANDLE hReadEnd;
        BOOL bRet = CreatePipe(&hReadEnd, &hWriteEnd, nullptr, PIPE_SIZE);
        GAME_ASSERT(bRet==TRUE, "CAudio error: CreatePipe failed!");
        hThread = CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE) MCIThread, hReadEnd, 0, &dwThreadID);
    ...
    void CAudio::ExecuteMciCommand(char* command) {
        ...
        if (strcmp(action, "pause") == 0) {
            ...
            mciSendString(statusCommand, rval, 200, nullptr);
        ...
        if (shouldExecuteCommand) {
            bool success = mciSendString(command, nullptr, 0, nullptr) == 0;
            //
    ...
    void CAudio::MCIThread(HANDLE hRead) {
        ...
        //
        while (ReadFile(hRead, ptr, buf + MAX_BUFFER_SIZE - ptr - 1, &dwRead, nullptr)) {
    ...
    void CAudio::SendMciCommand(char* command) {
        ...
        BOOL bRet = WriteFile(hWriteEnd, buf, strlen(buf), &dwWrite, nullptr);
    ...
    bool CAudio::Load(unsigned id, char* lpzFileName) {
        ...
        for (unsigned i = 0; i < file_type.size(); i++) file_type[i] = toupper(file_type[i]);
    ...
    bool CAudio::Open() {
        ...
        //
        isOpened = mciSendString("open mpegvideo", nullptr, 0, nullptr) == 0;
        if (isOpened)
            mciSendString("close mpegvideo", nullptr, 0, nullptr);
        return isOpened;
    }
    ...
    void CAudio::Play(unsigned id, bool repeat_flag) {
        ...
        //
        if (!info[id].isGood) return;
        char command[MAX_MCI_COMMAND_SIZE];
        if (repeat_flag) sprintf(command, "play device%d from 0 repeat", id);
        else sprintf(command, "play device%d from 0", id);
        SendMciCommand(command);
    }
