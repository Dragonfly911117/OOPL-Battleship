...
namespace game_framework {
    ...
    CGame::~CGame() { for (int i = 0; i < NUM_GAME_STATES; i++) delete gameStateTable[i]; }
    ...
    void CGame::OnFilePause() {
        if (ENABLE_GAME_PAUSE) {
            if (running) CAudio::Instance()->Pause();
            else CAudio::Instance()->Resume();
            running = !running;
        }
    ...
    void CGame::OnInitStates() {
        ...
        //
        for (int i = 0; i < NUM_GAME_STATES; i++) gameStateTable[i]->OnInit();
    }
    ...
    void CGame::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) { if (running) gameState->OnKeyUp(nChar, nRepCnt, nFlags); }

    void CGame::OnKillFocus() {
        CAudio::Instance()->Pause();
        if (ENABLE_GAME_PAUSE) running = false;
        if (CDDraw::IsFullScreen()) running = false;
    }

    void CGame::OnLButtonDown(UINT nFlags, CPoint point) { if (running) { gameState->OnLButtonDown(nFlags, point); } }

    void CGame::OnRButtonDown(UINT nFlags, CPoint point) { if (running) gameState->OnRButtonDown(nFlags, point); }

    void CGame::OnLButtonUp(UINT nFlags, CPoint point) { if (running) gameState->OnLButtonUp(nFlags, point); }

    void CGame::OnMouseMove(UINT nFlags, CPoint point) { if (running) gameState->OnMouseMove(nFlags, point); }

    void CGame::OnRButtonUp(UINT nFlags, CPoint point) { if (running) gameState->OnRButtonUp(nFlags, point); }
    ...
    void CDDraw::BltBackColor(DWORD color) {
        if (lpDDSBack) {
            if (lpDDSBack->IsLost()) RestoreSurface();
            DDBLTFX ddbltfx;
    ...
    void CDDraw::BltBackToPrimary() {
        if (!pCView || !lpDDSPrimary || !lpDDSBack) return;
        if (fullscreen) {
            CRect ClientRect;
            GetClientRect(ClientRect);
            if (lpDDSBack->IsLost()) RestoreSurface();
            if (lpDDSPrimary->IsLost()) RestoreSurface();
    ...
    void CDDraw::BltBitmapToBack(unsigned SurfaceID, int x, int y) {
        x = IsFullScreen() ? x + (RESOLUTION_X - SIZE_X) / 2 : x;
        y = IsFullScreen() ? y + (RESOLUTION_Y - SIZE_Y) / 2 : y;
        ...
        int blt_flag;
        if (BitmapColorKey[SurfaceID] != CLR_INVALID) blt_flag = DDBLT_WAIT | DDBLT_KEYSRC;
        else blt_flag = DDBLT_WAIT;
        if (lpDDSBack->IsLost()) RestoreSurface();
        if (lpDDS[SurfaceID]->IsLost()) RestoreSurface();
    ...
    void CDDraw::BltBitmapToBack(unsigned SurfaceID, int x, int y, double factor) {
        ...
        TargetRect.left = x;
        TargetRect.top = y;
        TargetRect.right = x + static_cast<int>((BitmapRect[SurfaceID].right - BitmapRect[SurfaceID].left) * factor);
    ...
    void CDDraw::BltBitmapToBitmap(unsigned SourceID, unsigned TargetID, int x, int y) {
        ...
        int blt_flag;
        if (BitmapColorKey[SourceID] != CLR_INVALID) blt_flag = DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY;
        else blt_flag = DDBLTFAST_WAIT;
        if (lpDDS[SourceID]->IsLost()) RestoreSurface();
        if (lpDDS[TargetID]->IsLost()) RestoreSurface();
    ...
    bool CDDraw::CreateSurface() {
        ...
        ReleaseSurface();
        if (fullscreen) { if (!CreateSurfaceFullScreen()) return false; } else { if (!CreateSurfaceWindowed()) return false; }
    ...
    DWORD CDDraw::MatchColorKey(LPDIRECTDRAWSURFACE lpDDSurface, COLORREF color) {
        ...
        DWORD dw = CLR_INVALID, mask = static_cast<DWORD>(~0);
        if (lpDDSurface && color != CLR_INVALID) {
            ...
            if (hres == DD_OK) {
                dw = *static_cast<DWORD*>(ddsd.lpSurface);// get (0,0) data
                if (ddsd.ddpfPixelFormat.dwRGBBitCount < 32) mask = (1 << ddsd.ddpfPixelFormat.dwRGBBitCount) - 1;
    ...
    void CGameState::ShowInitProgress(int percent, string message) {
        ...
        CTextDraw::Print(pDC, x1, static_cast<int>((SIZE_Y * 0.40)), message.c_str());
