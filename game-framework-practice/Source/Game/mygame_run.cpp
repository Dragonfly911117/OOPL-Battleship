...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
    if (nChar == VK_RETURN) {
        ...
        character.SetTopLeft(character.Left(), character.Top() - 25);
    }
    else if (nChar == VK_DOWN) {
        character.SetTopLeft(character.Left(), character.Top() + 25);
    }
    else if (nChar == VK_LEFT) {
        character.SetTopLeft(character.Left() - 25, character.Top());
    }
    ...
    if (phase <= 6) {
        ...
        if (phase == 3 && sub_phase == 1) {
            if (character.Top() + character.Height() >= chest_and_key.Top()
                && character.Left() + character.Width() <= chest_and_key.Left() + chest_and_key.Width()) {
        ...
        if (phase == 5 && sub_phase == 1) {
            for (int i = 0; i < 3; i++) {
                if (character.Top() >= door[i].Top() && character.Top() <= door[i].Top() + door[i].Height() &&
                    character.Left() >= door[i].Left() && character.Left() <= door[i].Left() + door[i].Width())
                    door[i].SelectShowBitmap(1);
                door[i].ShowBitmap();
            }
        }
        if (phase == 6) {
            if (sub_phase == 1 && ball.GetSelectShowBitmap() == 0) {
                ball.SetAnimation(1000, false);
            }
            else if (ball.GetSelectShowBitmap() == 3)
...
void CGameStateRun::show_text_by_phase() {
    ...
    CTextDraw::ChangeFontLog(pDC, fp, 21, "Ink free", RGB(0, 0, 0), 800);

    if (phase == 1 && sub_phase == 1) {
        CTextDraw::Print(pDC, 237, 128, "englishString and 中文字串");
