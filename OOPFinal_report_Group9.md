...
> 考量到掉幀對於遊玩體驗，我們的最佳化策略
>
> GameBoard::dropShip()會依照船隻大小檢查是否有足夠的格子能在不超出邊界或是與其他船隻重疊(BaseGrid底下有個_shipID屬性。其預設值是-1，並在有船隻被置於此處或撿起時做對應的更動。藉此，我們可以在O(船隻大小)的時間下而非遍歷整個棋盤便可知道是否有重疊)的情況下擺放。
...
5.  []{#_Toc137324710 .anchor}[1. 問題及解決方法（待詢問討論）]{.mark}

整學期所碰到的問題及解決方法。

6.  []{#_Toc137324711 .anchor}[2. 時間表（填寫時間）]{.mark}
